...
@property unsigned breakpointID;
...
@implementation GBDebuggerWindowController
{
    ...
    /* The disassembly covers the entire $0000-$ffff address space. When
       ...
       boundary. Each stop rebuilds the sweep to include memory changes. */
    ...
    NSNumber *_runToState; // nil when no Run to Cursor breakpoint is pending
}
...
/* Reads 1-3 instruction bytes and formats them the way the disassembly bytes
   column shows them. Requires the core to be accessible from the caller. */
- (NSString *)hexBytesAtAddress:(uint16_t)address length:(unsigned)length
{
    GB_gameboy_t *gb = _document.gb;
    char buffer[12] = "";
    char *p = buffer;
    for (unsigned i = 0; i < length; i++) {
        p += snprintf(p, buffer + sizeof(buffer) - p, i? " %02x" : "%02x",
                      GB_safe_read_memory(gb, address + i));
    }
    return @(buffer);
}
...
- (void)refreshLivePanes
{
    ...
    [document performAtomicBlock:^{
        ...
        for (NSUInteger i = visible.location; i < NSMaxRange(visible) && i < self->_disassemblyRows.count; i++) {
            ...
            NSString *newBytes = [self hexBytesAtAddress:row.address length:length];
...
/* Runs the block with the core safely accessible from the main thread. While
   the debugger is stopped, performAtomicBlock: synchronises with the emulation
   thread for us — isPaused alone is not enough, as it turns true as soon as
   the debugger is asked to stop, while the thread may still be on its way to
   (or back out of) getDebuggerInput. Otherwise the emulator is stopped for the
   duration of the block: these sweeps are far too long to stall the emulation
   thread with, unlike the short reads performAtomicBlock: is meant for. */
- (void)performWithCoreAccessible:(void (^)(void))block
{
    Document *document = _document;
    if (!document) return;
    if (document.isPaused) {
        [document performAtomicBlock:block];
        return;
    }
    [document stop];
    block();
...
- (void)toggleBreakpointAtAddress:(uint16_t)address
{
    ...
    [self performWithCoreAccessible:^{
        ...
        command = existing? [NSString stringWithFormat:@"delete %u", existing.breakpointID]
                                    : GBDebuggerBreakpointCommandForAddress(address, self->_currentBankContext);
        if (!paused) {
            executeDebuggerCommand(document.gb, command);
            [self reloadBreakpoints];
        }
    }];
    if (paused && command) [_document queueDebuggerCommand:command];
...
- (IBAction)runToCursor:(id)sender
{
    ...
    /* Own a separate unconditional breakpoint, even if the user has a
       conditional or jump-to breakpoint here. Keep its ID for cleanup. */
    __block NSString *output;
    [_document performAtomicBlock:^{
        [self updateCurrentBankContext];
        output = [self outputOfCommand:GBDebuggerBreakpointCommandForAddress(row.address, self->_currentBankContext)];
    }];
    unsigned breakpointID;
    if (sscanf(output.UTF8String ?: "", "Breakpoint %u set", &breakpointID) != 1) {
        NSBeep();
        return;
    }
    _runToState = @(breakpointID);
    [_document queueDebuggerCommand:@"continue"];
}
...
{
    /* Document -getDebuggerInput calls -updateDebuggerButtons on its link cable
       partner directly from the emulation thread, and that ends up here. */
...
- (void)debuggerDidRefresh
{
    ...
    if ((GB_debugger_is_stopped(document.gb) ||
         (document.partner && GB_debugger_is_stopped(document.partner.gb))) && !document.isDebuggerParked) {
    ...
    [document performAtomicBlock:^{
        /* Continue may have been consumed since the initial state check.
           Snapshot only while paused, and keep all core reads protected. */
        if (!document.isPaused) return;
        [self reloadRegistersLive:false];
        [self cleanUpRunToBreakpoint];
        [self reloadBreakpoints];
        [self reloadDisassembly];
        [self reloadStack];
        const char *description = GB_debugger_describe_address(document.gb, self->_pc, -1, false, true);
        status = [NSString stringWithFormat:@"Stopped at %s", description];
    }];
    if (!status) {
        [self updateRunningState];
        return;
...
- (void)updateCurrentBankContext
{
    ...
    if (!document) return;
    GB_gameboy_t *gb = document.gb;
    _currentBankContext.rom0Bank = GB_debugger_bank_for_address(gb, 0);
    _currentBankContext.romBank = GB_debugger_bank_for_address(gb, 0x4000);
    _currentBankContext.wramBank = GB_debugger_bank_for_address(gb, 0xd000);
}
...
- (void)reloadBreakpoints
{
    ...
    for (NSString *line in [output componentsSeparatedByString:@"\n"]) {
        ...
        breakpoint.breakpointID = (unsigned)idString.integerValue;
...
{
    /* The initial prompt may precede the queued continue. Only clean up
       after it has been consumed and execution stops again. */
    if (_document.hasPendingDebuggerInput) return;
    [self deleteRunToBreakpoint];
}
...
{
    NSNumber *state = _runToState;
    if (!state) return;
    _runToState = nil;

    [self outputOfCommand:[NSString stringWithFormat:@"delete %u", state.unsignedIntValue]];
}
...
- (void)rebuildDisassemblyWithAnchor:(int32_t)anchor
{
    ...
    if (anchor > 0) {
        ...
        for (uint32_t address = gapStart; address < (uint32_t)anchor; address++) {
            ...
            row.address = address;
            row.bytes = [self hexBytesAtAddress:address length:1];
            row.text = [@".BYTE $" stringByAppendingString:row.bytes];
    ...
    for (GBDisassemblyRow *row in rows) {
        ...
        if (previous && !previous.bytes) {
            ...
            if (length >= 1 && length <= 3) {
                previous.bytes = [self hexBytesAtAddress:previous.address length:length];
...
{
    /* ponytail: rebuild at every stop so edits, state loads and RAM code
       cannot leave stale instruction boundaries; add content-aware caching
       if full sweeps become a stepping bottleneck. */
...
- (void)reloadStack
{
    ...
    for (unsigned i = 0; i < GBStackRowCount; i++) {
        ...
        /* Stop before $ffff too: its high byte lives at $10000, and reading it
           would wrap around to $0000 rather than continue the stack. */
        if (address < sp || address == 0xFFFF) break;
...
- (id)tableView:(NSTableView *)tableView objectValueForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    if (tableView == _disassemblyTable) {
        ...
        if ([tableColumn.identifier isEqualToString:@"gutter"]) {
            if (rowData.isLabel) return @"";
            NSString *marker = [self breakpointAtAddress:rowData.address]? @"●" : @"";
...
- (void)tableView:(NSTableView *)tableView willDisplayCell:(id)cell forTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if (tableView == _disassemblyTable && (NSUInteger)row < _disassemblyRows.count) {
        ...
        if ([tableColumn.identifier isEqualToString:@"gutter"] && !rowData.isLabel &&
            [self breakpointAtAddress:rowData.address]) {
