...
static void buildTables(void)
{
    ...
    static const struct {const char *pattern; uint8_t opcode;} singles[] = {
        ...
        {"jp hl", 0xE9}, {"jp [hl]", 0xE9}, {"ld sp,hl", 0xF9},
        {"ld [bc],a", 0x02}, {"ld [de],a", 0x12}, {"ld a,[bc]", 0x0A}, {"ld a,[de]", 0x1A},
        {"ld [hli],a", 0x22}, {"ld [hld],a", 0x32}, {"ld a,[hli]", 0x2A}, {"ld a,[hld]", 0x3A},
        {"ld [hl+],a", 0x22}, {"ld [hl-],a", 0x32}, {"ld a,[hl+]", 0x2A}, {"ld a,[hl-]", 0x3A},
        {"ldi [hl],a", 0x22}, {"ldd [hl],a", 0x32}, {"ldi a,[hl]", 0x2A}, {"ldd a,[hl]", 0x3A},
        {"ldh [c],a", 0xE2}, {"ldh a,[c]", 0xF2}, {"ld [c],a", 0xE2}, {"ld a,[c]", 0xF2},
    };
    ...
    for (unsigned i = 0; i < 4; i++) {
        ...
        addLiteralOpcode([NSString stringWithFormat:@"add hl,%@", rr], 0x09 + i * 0x10);
    ...
    for (unsigned i = 0; i < 8; i++) {
        ...
        for (unsigned op = 0; op < 8; op++) {
            ...
            addLiteralOpcode([NSString stringWithFormat:@"%s a,%@", aluOps[op], reg], opcode);
        }
    ...
    for (unsigned op = 0; op < 8; op++) {
        ...
        addTemplate([NSString stringWithFormat:@"%s a,\x01", aluOps[op]], GBOperandImm8, 0xC6 + op * 8);
    }

    /* RST vectors, in the disassembler's spelling */
    for (unsigned i = 0; i < 8; i++) {
    ...
    addTemplate(@"ldh a,[\x01]", GBOperandImm8High, 0xF0);
    addTemplate(@"add sp,\x01", GBOperandSign8, 0xE8);
    addTemplate(@"ld hl,sp,\x01", GBOperandSign8, 0xF8);
    addTemplate(@"ld hl,sp+\x01", GBOperandSign8, 0xF8);
    addTemplate(@"ld hl,sp-\x01", GBOperandSign8Negated, 0xF8);
    addTemplate(@"ld [\x01],a", GBOperandImm16, 0xEA);
    addTemplate(@"ld a,[\x01]", GBOperandImm16, 0xFA);
...
/* Strips comments and redundant whitespace, and maps parentheses to brackets,
   without changing character content otherwise (case is preserved so symbol
   operands survive). The single space separating the mnemonic from its first
   operand is kept, and every pattern carries it: stripping it would collapse
   "RLC A" onto the literal "RLCA" and assemble $07 instead of $cb $07 — same
   for RL/RRC/RR A and CP L. */
static NSString *canonicalize(NSString *input)
{
    ...
    NSRange separator = [output rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
    if (separator.location == NSNotFound) return output;
    NSString *operands = [[[output substringFromIndex:separator.location]
                           componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]
                          componentsJoinedByString:@""];
    return [NSString stringWithFormat:@"%@ %@", [output substringToIndex:separator.location], operands];
}
...
{
    return [@[@"a", @"f", @"b", @"c", @"d", @"e", @"h", @"l",
              @"af", @"bc", @"de", @"hl", @"sp", @"pc"] containsObject:operand.lowercaseString];
}

/* -1 for anything that is not an ASCII hex digit. The 0x7F guard matters:
   without it a non-ASCII unichar such as U+0430 truncates to '0' and would be
   accepted as a digit. */
...
bool GBSM83ParseHexLiteral(NSString *expression, uint16_t *value)
{
    ...
    for (NSUInteger i = 0; i < hexPart.length; i++) {
        signed digit = hexDigitValue([hexPart characterAtIndex:i]);
...
NSData *GBSM83Assemble(NSString *instruction, uint16_t address, GBSM83OperandEvaluator evaluator, NSString **error)
{
    ...
    /* Templates locate their operand by index in `lowered` and then slice it
       out of `canonical` to keep symbol case. Case folding can change the
       UTF-16 length (U+0130 folds to two units), which would make that slice
       range invalid; no SM83 mnemonic or operand needs such a character. */
    if (lowered.length != canonical.length) {
    ...
    for (GBSM83Template *entry in templates) {
        ...
        switch (entry.kind) {
            ...
            case GBOperandImm16:
                ...
                break;
            case GBOperandRel8: {
                int16_t offset = (uint16_t)(value - address - 2);
