...
int main(void)
{
    @autoreleasepool {
        ...
        expectEqualUnsigned(GBDebuggerBankForAddress(0xf123, context), 0x00,
                            "echo RAM uses bank zero like the core debugger");
        ...
        expectTrue(!GBDebuggerBreakpointMatchesAddress(0x4000, 0x03, 0x4000, context),
                   "banked breakpoints do not match the same address in another bank");
        expectTrue(!GBDebuggerBreakpointMatchesAddress(0x4001, GBDebuggerAnyBank, 0x4000, context),
        ...
        expectTrue(!rows[0].isLabel, "instruction rows are not labels");
