...
int main(void)
{
    @autoreleasepool {
        ...
        /* The mnemonic/operand separator must survive canonicalization: these
           CB forms are otherwise indistinguishable from the one-byte literals
           they collapse onto, which win the table lookup. */
        EXPECT("RLC A", 0, 0xCB, 0x07);  EXPECT("RLCA", 0, 0x07);
        EXPECT("RRC A", 0, 0xCB, 0x0F);  EXPECT("RRCA", 0, 0x0F);
        EXPECT("RL A",  0, 0xCB, 0x17);  EXPECT("RLA",  0, 0x17);
        EXPECT("RR A",  0, 0xCB, 0x1F);  EXPECT("RRA",  0, 0x1F);
        EXPECT("CP L",  0, 0xBD);        EXPECT("CPL",  0, 0x2F);
        ...
        EXPECT("ld a, [$1234]", 0, 0xFA, 0x34, 0x12);
        ...
        EXPECT("jr $0000", 0xfffd, 0x18, 0x01);
        EXPECT("jr $ffff", 0x0000, 0x18, 0xfd);
        EXPECT("jr $0080", 0xffff, 0x18, 0x7f);
        EXPECT("jr $ff81", 0xffff, 0x18, 0x80);
        expectError(@"jr $0081", 0xffff, NULL);
        expectError(@"jr $ff80", 0xffff, NULL);
        ...
        /* Characters whose lowercase is longer than the original must be
           rejected rather than throwing out of the operand slice */
        expectError(@".byteİ", 0, NULL);
        expectError(@"LD A, İ", 0, NULL);
        expectError(@"jp İ", 0, evaluator);

        /* Non-ASCII characters must not pass for hex digits by truncating to
           an ASCII one — "а" is U+0430, whose low byte is '0' */
        uint16_t parsed = 0;
        expectTrue(!GBSM83ParseHexLiteral(@"аа", &parsed),
                   "non-ASCII lookalikes are not hex literals");
        expectTrue(GBSM83ParseHexLiteral(@"$4008", &parsed) && parsed == 0x4008,
                   "hex literals parse with a $ prefix");
        expectTrue(GBSM83ParseHexLiteral(@"ff", &parsed) && parsed == 0x00ff,
                   "hex literals parse without a prefix");
        expectTrue(!GBSM83ParseHexLiteral(@"12345", &parsed),
                   "hex literals longer than 4 digits are rejected");
        /* Such an operand must reach the evaluator instead of silently
           assembling as $0000 */
        const uint8_t jpNonASCII[] = {0xC3, 0x34, 0x12};
        expectBytes(@"jp аа", 0, ^bool(NSString *expression, uint16_t *value) {
            *value = 0x1234;
            return true;
        }, jpNonASCII, sizeof(jpNonASCII));
